...
import {
  ...
  Wallet, TrendingUp, Home, CreditCard, LayoutGrid,
} from "lucide-react";
...
const SECTIONS = [
  {
    groupKey: "more.group_finance_tools",
    items: [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/profile",       icon: User,     labelKey: "nav.profile",       descKey: "more.desc_profile",       color: "text-gray-400",   bg: "bg-gray-400/10"         },
...
function NavRow({
  href, icon: Icon, labelKey, descKey, color, bg, badge, unread, isActive,
}: {
  href: string; icon: React.ElementType; labelKey: string; descKey: string;
  color: string; bg: string; badge?: boolean; unread?: number; isActive: boolean;
}) {
  const { t } = useTranslation();
  return (
    <Link
      href={href}
      className={`flex items-center gap-4 px-4 py-3.5 transition-colors border-b border-[hsl(var(--border-2))] last:border-0 ${
