...
import {
  Bell, Check, ChevronRight, Database, Download, Eye, EyeOff,
  Globe, KeyRound, Loader2, Lock, LockKeyhole, LogIn, LogOut,
  Moon, Palette, Shield, Sparkles, Star, Sun, Trash2, Unlock,
  User, WalletCards,
} from "lucide-react";
  ...
  return (
    ...
    >
      <span className={`h-4 w-4 rounded-full bg-white shadow-sm transition-transform ${
        checked ? "translate-x-[20px] rtl:translate-x-[4px]" : "translate-x-1 rtl:translate-x-[20px]"
      }`} />
    </motion.button>
  );
}
...
type Tab = "account" | "system" | "subscription" | "privacy";
...
export default function SettingsPage() {
  ...
  const TABS: { id: Tab; label: string; icon: React.ElementType }[] = [
    { id: "account",      label: t("more.group_account"),       icon: User    },
    { id: "system",       label: t("more.group_system"),        icon: Palette },
    { id: "subscription", label: t("more.group_subscriptions"), icon: Star    },
    ...
    <div className="space-y-4 max-w-2xl mx-auto">
      ...
      <div className="sticky top-0 z-20 -mx-3 border-b border-[hsl(var(--border-2))] bg-[hsl(var(--bg))]/95 px-3 py-3 backdrop-blur-xl sm:-mx-5 sm:px-5">
        <div className="flex items-center justify-between gap-3">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.96 }}
            transition={tapTransition}
            className={`inline-flex min-h-9 shrink-0 items-center gap-1.5 rounded-xl px-4 text-xs font-bold transition-all disabled:opacity-50 ${
              saved
      ...
      <div className="card overflow-hidden">
        <div className="flex items-center gap-4 p-4">
          <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-cyan-500 text-base font-bold text-white">
            {isGuest ? <User className="h-5 w-5" /> : initials}
          </div>
          <div className="min-w-0 flex-1">
      ...
      <AnimatePresence mode="wait">
        ...
        >
                ...
                <Section title={t("settings.change_password")} icon={KeyRound} color="text-violet-300" bg="bg-violet-400/10">
                  <div className="p-4 space-y-3">
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.97 }}
                      transition={tapTransition}
                      className="flex w-full items-center justify-center gap-2 rounded-xl bg-violet-500 py-2.5 text-sm font-bold text-white disabled:opacity-40"
                    >
                    ...
                    {importDone ? (
                      ...
                      <motion.button
                        ...
                        onClick={async () => {
                          ...
                          setHasGuestData(false); setImportDone(true); setImporting(false);
                          setTimeout(() => setImportDone(false), 3000);
                        }}
                        disabled={importing}
                        whileTap={{ scale: 0.97 }}
                        transition={tapTransition}
                        className="flex w-full items-center gap-3 rounded-xl border border-cyan-400/25 bg-cyan-400/10 px-4 py-3 text-sm font-semibold text-cyan-300 disabled:opacity-50"
                      >
                ...
                <Section title={t("nav.sign_out")} icon={LogOut} color="text-rose-300" bg="bg-rose-400/10">
                  <div className="p-4">
                    <motion.button
                      ...
                      whileTap={{ scale: 0.97 }}
                      transition={tapTransition}
                      className="flex w-full items-center justify-center gap-2 rounded-xl border border-rose-400/25 bg-rose-400/10 py-2.5 text-sm font-bold text-rose-300 disabled:opacity-50"
                    >
                    ...
                    <motion.button
                      ...
                      className={`flex flex-col items-center gap-2 rounded-xl border py-4 transition-all ${
                        theme === id
                          ...
                          : "border-[hsl(var(--border-2))] bg-[hsl(var(--bg-card-2))]"
                      }`}
                    >
                      ...
                      {theme === id && <Check className="h-3 w-3 text-violet-300" />}
                    </motion.button>
                  ))}
                </div>
              </Section>
          ...
          {tab === "subscription" && (
            <>
              <Link href="/plans"
                className="card flex items-center gap-4 p-4 hover:border-cyan-400/20 transition-all group">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-cyan-400/20 to-purple-400/20">
                  <Star className="h-5 w-5 text-cyan-300" />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-bold t1">{t("nav.plans")}</p>
                  <p className="text-xs t3 mt-0.5">{t("more.desc_subscriptions")}</p>
                </div>
                <ChevronRight className="h-4 w-4 t3 shrink-0 group-hover:text-cyan-300 transition-colors" />
              </Link>

              <Section title={t("more.group_subscriptions")} icon={WalletCards} color="text-violet-300" bg="bg-violet-400/10">
                <div className="p-4 space-y-3">
                  <p className="text-xs t3">{t("more.desc_subscriptions")}</p>
                  <Link href="/subscriptions"
                    className="flex w-full items-center justify-center gap-2 rounded-xl border border-violet-400/25 bg-violet-400/10 py-2.5 text-sm font-bold text-violet-300 transition-all hover:bg-violet-400/15">
          ...
          {tab === "privacy" && (
            <>
              <Section title={t("rooms.pin_title")} icon={KeyRound} color="text-cyan-300" bg="bg-cyan-400/10">
                <div className="p-4 space-y-3">
                  <div className="flex items-center gap-3 rounded-xl border border-[hsl(var(--border-2))] bg-[hsl(var(--bg-card-2))] px-3 py-2.5">
                    {hasPin
                  ...
                  <form onSubmit={handlePinSubmit} className="flex gap-2">
                    <input
                      value={roomPin}
                      onChange={(e) => { setRoomPin(e.target.value); if (pinError) setPinError(""); }}
                      type="password"
                      inputMode="numeric"
                    ...
                    />
                    <button type="submit"
                      className="min-h-10 rounded-xl bg-cyan-400 px-4 text-xs font-bold text-[#071018]">
              ...
              <Section title={t("settings.security")} icon={Shield} color="text-slate-300" bg="bg-slate-400/10">
                {ROOM_DEFINITIONS.map((room) => (
                  <SettingRow key={room.id} label={t(room.labelKey)} hint={t(room.descriptionKey)}>
                    <Toggle
                      ...
                      onChange={(v) => setRoomLocked(room.id, v)}
                      disabled={!hasPin}
                    />
                  </SettingRow>
                ))}
                <div className="p-3 pt-1">
                  <button
                    ...
                    disabled={!hasPin || lockedCount === ROOM_DEFINITIONS.length}
                    className="flex w-full items-center justify-center gap-2 rounded-xl border border-amber-400/20 bg-amber-400/10 py-2.5 text-xs font-bold text-amber-300 disabled:opacity-40 transition-all"
                  >
              ...
              <Link href="/privacy"
                className="card flex items-center gap-3 p-4 hover:border-cyan-400/20 transition-all group">
                <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-400/10 shrink-0">
                  ...
                  <div className="p-3 space-y-2">
                    {(["transactions", "goals", "debts"] as const).map((type) => (
                      <motion.button
                        ...
                        whileTap={{ scale: 0.97 }}
                        transition={tapTransition}
                        className="flex w-full items-center gap-3 rounded-xl border border-emerald-400/20 bg-emerald-400/8 px-4 py-3 text-sm font-semibold text-emerald-300 disabled:opacity-50 transition-all hover:bg-emerald-400/12"
                      >
                    ...
                    <a href="/api/export?type=backup" download
                      className="flex w-full items-center gap-3 rounded-xl border border-cyan-400/20 bg-cyan-400/8 px-4 py-3 text-sm font-semibold text-cyan-300 transition-all hover:bg-cyan-400/12">
                  ...
                  <div className="p-4 space-y-3">
                    {deleteStep === "idle" && (
                      <motion.button
                        ...
                        whileTap={{ scale: 0.98 }}
                        transition={tapTransition}
                        className="flex w-full items-center gap-3 rounded-xl border border-rose-400/25 bg-rose-400/10 px-4 py-3 text-sm font-semibold text-rose-300 transition-all"
                      >
                    ...
                    {deleteStep === "export" && (
                      <div className="space-y-3 rounded-xl border border-amber-400/30 bg-amber-400/8 p-4">
                    ...
                    {deleteStep === "confirm" && (
                      <div className="space-y-3 rounded-xl border border-rose-400/30 bg-rose-400/10 p-4">
